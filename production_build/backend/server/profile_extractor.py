...
BUDGET_PATTERN = re.compile(r"(\$?\d{1,3}(?:,\d{3})*(?:k|000)?(?:\s*max)?)", re.IGNORECASE)
LOCATION_KEYWORDS = {"cary", "raleigh", "durham", "apex", "chapel hill", "wake forest", "morrisville", "holly springs", "fuquay"}
...
def extract_profile_fields(message: str) -> Dict[str, str]:
    ...
    if budget_match and any(c.isdigit() for c in budget_match.group(1)):
         # weak check but works for "700k"
