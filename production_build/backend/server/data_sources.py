...
def _to_int(value: Any) -> int:
    ...
    try:
        ...
        # Handle shorthand like "700k" or "1.2m"
        multiplier = 1
        ...
        cleaned = cleaned.replace("$", "").replace(",", "").replace(" max", "").strip()
        if not cleaned:
            return 0
...
def search_listings(
    ...
    def location_match(item: Dict[str, Any]) -> bool:
        ...
        # If we have no structured location fields, don't filter out the listing
        if not any(city_fields):
            return True
        fields = [item.get("address", "")] + city_fields
