...
# Ensure extract_profile_fields is always defined, even if the helper module is missing in prod.
def extract_profile_fields(message: str) -> Dict[str, str]:
    return {}

try:
    from .profile_extractor import extract_profile_fields as _extract_profile_fields
    extract_profile_fields = _extract_profile_fields  # type: ignore
except Exception as exc:  # pragma: no cover - defensive fallback for missing file on deploy
    print(f"[warn] extract_profile_fields import failed: {exc}; using no-op extractor")
...
DEFAULT_SYSTEM_PROMPT = """You are the digital receptionist for [BUSINESS NAME]. Your goal is to help visitors find a home and qualify them for a tour.

Guidelines:
- Tone: Be warm, professional, and helpful.
- Style: NEVER use double dashes (--) or em-dashes (—).
- Discovery: Don't interrogate. Ask comprehensive questions like "What city or neighborhood are you looking in, and do you have a specific budget or number of bedrooms in mind?"
- memory: Use [Profile Context] to avoid asking for 'Known' info.
- CRITICAL: Present properties ONLY using the <listing-card> tag. NEVER write beds/baths/price in raw text.
- Ask one clear question at a time to guide the conversation forward."""
TOOLING_PROMPT = """For property specs or address requests, call lookup_listings. 
When presenting results, ALWAYS use the <listing-card> tag.
Format: <listing-card address="[Addr]" price="[Price]" beds="[N]" baths="[N]" sqft="[N]" image="[URL]" link="[URL]"></listing-card>
Do not write specs in the text. Always include a brief, friendly sentence before or after the card.
If no listing matches, ask for budget/beds/location and offer team follow-up.
Do not call log_lead unless you have name AND email/phone AND a property/interest."""
...
def _safe_extract_profile_fields(message: str) -> Dict[str, str]:
    """Robust wrapper so missing import never crashes prod."""
    try:
        return extract_profile_fields(message)
    except Exception:
        return {}
...
def _get_runtime_config(sheet_id_override: Optional[str] = None) -> Dict[str, Any]:
    ...
    sheet_env = (os.getenv("SHEETS_SPREADSHEET_ID") or "").strip()
    sheet_id = (sheet_id_override or "").strip() or sheet_env or "1noWqSb893DyF9n7ZQEcs-ItJtG3aZnVY7BPaiz8KzUg"
    return {
        "api_key": api_key,
...
def load_system_prompt(sheet_id_override: Optional[str] = None) -> str:
    sheet_id = sheet_id_override or (os.getenv("SHEETS_SPREADSHEET_ID") or "").strip() or "1noWqSb893DyF9n7ZQEcs-ItJtG3aZnVY7BPaiz8KzUg"
    ...
    try:
        ...
        # Fallback to CWD if not in server dir
        try:
            file_prompt = Path("prompt.txt").read_text(encoding="utf-8").strip()
        except FileNotFoundError:
            file_prompt = ""
...
def determine_stage_and_focus(profile: Dict[str, Any]) -> Tuple[str, str]:
    ...
    # Need is known if we have a summary, product/interest, OR a specific location/budget
    need_known = bool(profile.get("product_name") or profile.get("summary") or profile.get("location") or profile.get("budget"))
    
    # Constraints are known if we have location
    constraints_known = bool(profile.get("location"))
    ...
    if not constraints_known and not budget_known:
...
def build_profile_prompt(profile: Dict[str, Any], stage: str, focus: str) -> str:
    ...
    if profile.get("agent_status"):
        known.append(f"Has Agent: {profile['agent_status']}")
    if profile.get("pre_approval"):
        known.append(f"Pre-approved: {profile['pre_approval']}")
    if profile.get("location"):
        known.append(f"Location: {profile['location']}")
    if profile.get("budget"):
        known.append(f"Budget: {profile['budget']}")
    if profile.get("product_name"):
        known.append(f"Interest: {profile['product_name']}")
...
def build_site_context(sheet_id_override: Optional[str] = None) -> str:
    ...
    if listings:
        ...
        for listing in listings[:10]:
            img = listing.get('images', [])[0] if listing.get('images') else ""
            listing_lines.append(
                ...
                f" | acres={listing.get('acres','')} | image={img} | url={listing.get('listing_url','')}"
            )
...
def update_conversation_profile(
    ...
    # Save generic fields only
    allowed_fields = {
        "stage", "intent", "urgency", "contact_name", "agent_status", "pre_approval",
        "location", "budget", "product_name", "requested_date",
        "contact_email", "contact_phone", "summary"
    }
    
    storage.upsert_profile(
        conversation_id,
        {k: v for k, v in merged.items() if k in allowed_fields and v},
    )
...
def _dispatch_tool_call(
    ...
    if call["name"] == "lookup_listings":
        # Capture location/budget from tool usage
        updates = {}
        if args.get("location"):
            updates["location"] = args["location"]
        if args.get("price_max"):
            updates["budget"] = str(args["price_max"])
        
        if updates:
            curr = storage.get_profile(conversation_id) or {}
            curr.update(updates)
            storage.upsert_profile(conversation_id, curr)
    ...
    if call["name"] == "log_lead":
        return _tool_log_lead(args, conversation_id, profile, sheet_id_override)
...
async def chat_endpoint(payload: ChatRequest) -> StreamingResponse:
    ...
    extracted_fields = _safe_extract_profile_fields(payload.message)
    ...
    async def response_generator():
        ...
        if use_tools:
            # Inline expansion of _run_with_tools to support yielding status
            ...
            for _ in range(3):
                ...
                try:
                    result = ai_client.create_chat_completion(
                        tool_messages,
                        tools=TOOL_DEFINITIONS_PRIMARY + TOOL_DEFINITIONS_ROUTING,
                        tool_choice="auto",
                    )
                except Exception as exc:
                ...
                message = result["choices"][0]["message"]
                current_text = _strip_tool_call_blocks(message.get("content") or "")
                tool_calls = _parse_tool_calls(message)
                
                if not tool_calls:
                    raw_text = current_text if current_text else last_text_local
                    break
                
                if current_text:
                    last_text_local = current_text

                # 2. Yield Status for Tools
                import random
                for call in tool_calls:
                    t_name = call["name"]
                    status_msg = "Processing..."
                    if t_name == "lookup_listings":
                        status_msg = random.choice(["Searching listings...", "Checking inventory...", "Finding matches...", "Looking up homes..."])
                    elif t_name == "log_lead":
                        status_msg = random.choice(["Saving details...", "Updating profile...", "Noting that down..."])
                ...
                for call in tool_calls:
                    tool_result = _dispatch_tool_call(call, conversation_id, existing_profile or {}, sheet_id)
                    tool_events.append({
                        "name": call["name"],
                        "arguments": call["arguments"],
                        "result": tool_result,
            ...
            # No tools (fake mode or configured off)
            raw_text = ai_client.generate(llm_messages)

        # Final Processing
        ai_result: AIResult = parse_response(raw_text)
        tool_routing = _extract_routing_from_events(tool_events)
        
        # (Optional) Run routing tool if needed
        if use_tools and not tool_routing:
             # Yield status for routing
