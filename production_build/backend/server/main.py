...
DEFAULT_SYSTEM_PROMPT = """You are an after-hours AI receptionist. Non-negotiables:
- Never invent availability, pricing, or offerings. Only use details supplied in the Sheet-powered context (Availability, Inventory, Settings).
- Never invent sizes or variants. If a size is not provided in inventory results, say you will confirm with the team.
- Do not mention specific inventory items or availability slots unless the visitor asks about products/services or scheduling.
...
TOOLING_PROMPT = """If the [Inventory] section is empty or says none, do not ask for a product SKU. Ask the visitor what they need, keep it to this business, and collect contact + timing before promising specifics.
If a user asks for unrelated work (e.g., homework or writing a paper), state you are the business's receptionist and steer them back to what services/products they need from this business.
Never promise inventory or discounts that are not in [Inventory]."""
...
def _load_tenant_configs(force: bool = False) -> Dict[str, Dict[str, Any]]:
    now = time.time()
    if not force and TENANT_CACHE["data"] and now - TENANT_CACHE["loaded_at"] < TENANT_CACHE_TTL:
        return TENANT_CACHE["data"]
    sheet_id = os.getenv("SHEETS_SPREADSHEET_ID")
    configs: Dict[str, Dict[str, Any]] = {}
    if sheet_id:
        rows = read_sheet_dicts(sheet_id, TENANT_KEYS_RANGE)
        for row in rows:
            business_id = (row.get("business_id") or "").strip()
            api_key = (row.get("openai_api_key") or row.get("api_key") or "").strip()
            model = (row.get("model") or MODEL_NAME).strip()
            status = (row.get("status") or "active").strip().lower()
            rate_limit = (row.get("rate_limit_per_min") or "").strip()
            sheet_id = (row.get("sheet_id") or row.get("spreadsheet_id") or "").strip()
    ...
    TENANT_CACHE["data"] = configs
    TENANT_CACHE["loaded_at"] = now
    return configs


def _get_tenant_config(business_id: str) -> Dict[str, Any]:
    if not business_id:
        raise HTTPException(status_code=400, detail="business_id is required")
    configs = _load_tenant_configs()
    cfg = configs.get(business_id)
    if cfg:
        if cfg.get("status", "active") != "active":
            raise HTTPException(status_code=403, detail="Tenant inactive")
        return cfg
    # Fallback to single-tenant env key if provided
    env_key = os.getenv("OPENAI_API_KEY")
    if env_key:
        return {"api_key": env_key, "model": os.getenv("MODEL_NAME", MODEL_NAME), "status": "active"}
    raise HTTPException(status_code=401, detail="Unknown business_id")
...
TENANT_CACHE: Dict[str, Any] = {"data": {}, "loaded_at": 0}
TENANT_CACHE_TTL = int(os.getenv("TENANT_KEYS_CACHE_TTL", "300"))
TENANT_KEYS_RANGE = os.getenv("TENANT_KEYS_RANGE", "Keys!A:G")
...
TOOL_DEFINITIONS_PRIMARY = [
    {
        ...
        "function": {
            ...
            "description": "Look up inventory by SKU or a short product query. Results include sizes parsed from keywords when available.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Product description or size request from the visitor.",
                    },
                    "sku": {
                        "type": "string",
                        "description": "Exact SKU if already known.",
                    },
                    "limit": {
                        "type": "integer",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "preferred_time": {"type": "string", "description": "Preferred time window or pickup time"},
                    "intent": {"type": "string"},
                    "urgency": {"type": "string"},
                    "summary": {"type": "string"},
...
SIZE_FROM_KEYWORD_RE = re.compile(r"\bsize\s*([0-9]{1,2})\b", re.IGNORECASE)
DIGIT_ONLY_RE = re.compile(r"^\d{1,2}$")
...
def _extract_sizes_from_keywords(keywords: list[str]) -> list[str]:
    sizes = set()
    for keyword in keywords:
        if not keyword:
            continue
        match = SIZE_FROM_KEYWORD_RE.search(keyword)
        if match:
            sizes.add(match.group(1))
            continue
        token = keyword.strip()
        if DIGIT_ONLY_RE.match(token):
            sizes.add(token)
    return sorted(sizes, key=lambda value: int(value))
...
def build_profile_prompt(profile: Dict[str, Any], stage: str, focus: str) -> str:
    ...
    return (
        ...
        f"{checklist}\n"
        "Guidelines: Ask exactly one question, keep ≤3 short sentences, avoid filler closings, and mention availability only if the visitor is scheduling."
    )
...
def build_availability_context(
    ...
    if not allow_schedule:
        return "", extracted
    lowered = message.lower()
    needs_schedule = any(keyword in lowered for keyword in SCHEDULING_KEYWORDS)
    requested_date = extracted.get("requested_date")
    note = ""
    availability = load_availability(sheet_id_override)

    if availability:
        lines = [f"- {entry['date']}: {', '.join(entry['slots'])}" for entry in availability[:5]]
        note = "Use these exact date/time options:\n" + "\n".join(lines)
        if requested_date and not find_slots_for_date(requested_date, sheet_id_override):
            note += f"\nRequested date {requested_date} not found; use provided slots instead."

    return note, extracted


def build_site_context(include_inventory: bool = True, sheet_id_override: Optional[str] = None) -> str:
    parts = []
    sheet_id = sheet_id_override or os.getenv("SHEETS_SPREADSHEET_ID")
    prompt_range = os.getenv("SHEETS_PROMPT_RANGE", "Settings!A:C")
    fallback_about = os.getenv(
        "FALLBACK_ABOUT",
        "We provide an after-hours AI receptionist that answers FAQs, checks availability, and captures contact details for follow-up.",
    )
    if sheet_id:
        rows = read_sheet_dicts(sheet_id, prompt_range)
        for row in rows:
    ...
    availability = load_availability(sheet_id)
    if availability:
        lines = [f"- {entry['date']}: {', '.join(entry['slots'])}" for entry in availability]
        parts.append("[Availability]\n" + "\n".join(lines))

    if include_inventory:
        inventory = load_inventory(sheet_id)
        if inventory:
            inv_lines = []
            for item in inventory:
                inv_lines.append(
                    f"- {item.get('sku','')}: {item.get('name','')} | status={item.get('status','')}"
                    f" | available={item.get('available','')} | eta={item.get('eta','')}"
                    f" | keywords={', '.join(item.get('keywords', []) or [])}"
                    f" | price_band={item.get('price_band','')}"
                )
            parts.append("[Inventory]\n" + "\n".join(inv_lines))
        else:
            parts.append("[Inventory]\n- none loaded; ask what they need, keep it general, and collect contact + timing.")
...
def build_inventory_context(
    ...
    inventory = load_inventory(sheet_id_override)
    fields = extracted.copy()
    match = find_inventory_match(message, inventory)
    if not match and existing_profile.get("product_sku"):
        match = next((item for item in inventory if item.get("sku") == existing_profile.get("product_sku")), None)
    if match:
        status = match.get("status", "unknown")
        available = match.get("available", 0)
        if status == "in_stock" and available > 0:
            fields.setdefault("inventory_status", "in_stock")
        elif status == "service":
            fields.setdefault("inventory_status", "service")
        else:
            fields.setdefault("inventory_status", "backorder")
        fields.setdefault("product_sku", match.get("sku"))
        fields.setdefault("product_name", match.get("name"))
        note = format_inventory_note(match)
        return note, fields
    return "", fields
...
def _tool_lookup_inventory(args: Dict[str, Any], sheet_id_override: Optional[str] = None) -> Dict[str, Any]:
    inventory = load_inventory(sheet_id_override)
    ...
    if sku:
        match = next((item for item in inventory if item.get("sku") == sku), None)
        if match:
            items = [match]
    if not items and query:
        match = find_inventory_match(query, inventory)
        if match:
            items = [match]
    if not items and query:
        lowered = query.lower()
        ...
        "items": [
            {
                ...
                "sizes": _extract_sizes_from_keywords(item.get("keywords") or []),
            }
            for item in items
        ],
    }
...
def _dispatch_tool_call(
    ...
    if call["name"] == "lookup_inventory":
        return _tool_lookup_inventory(args, sheet_id_override)
...
def availability_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid from date. Use YYYY-MM-DD.") from exc
    data = filter_availability(start_dt, days)
    return {"data": data}


@app.get("/api/inventory")
def inventory_endpoint(sku: Optional[str] = Query(None)) -> Dict[str, Any]:
    inventory = load_inventory()
    if sku:
        inventory = [item for item in inventory if item.get("sku") == sku]
    return {"data": inventory}


@app.post("/api/chat", response_model=ChatResponse)
def chat_endpoint(payload: ChatRequest) -> ChatResponse:
    tenant_cfg = _get_tenant_config(payload.business_id)
    sheet_override = (payload.sheet_id or "").strip() or None
    tenant_sheet_id = sheet_override or tenant_cfg.get("sheet_id") or os.getenv("SHEETS_SPREADSHEET_ID")
    print(f"[chat] business_id={payload.business_id} sheet_override={sheet_override or 'none'} tenant_sheet_id={tenant_sheet_id or 'none'}")
    system_prompt = load_system_prompt(tenant_sheet_id)
    ai_client = build_ai_client(
        AI_PROVIDER,
        model_name=tenant_cfg.get("model", MODEL_NAME),
        api_key_override=tenant_cfg["api_key"],
    )
    ...
    _, extracted_fields = build_availability_context(payload.message, extracted_fields, allow_schedule, tenant_sheet_id)
    _, extracted_fields = build_inventory_context(payload.message, extracted_fields, existing_profile or {}, tenant_sheet_id)
    ...
    site_context = build_site_context(include_inventory=True, sheet_id_override=tenant_sheet_id)
    llm_messages = (
    ...
    if lead_event:
        ...
        if lead_result.get("ok") is False:
            ...
            clean_reply = f"Thanks {c_name}. We'll contact you at {c_contact}. Anything else you’d like to add?"
...
def lead_endpoint(payload: LeadRequest) -> LeadResponse:
    cfg = _get_tenant_config(payload.business_id)  # validates tenant exists/active
    sheet_override = (payload.sheet_id or "").strip() or None
    tenant_sheet_id = sheet_override or cfg.get("sheet_id") or os.getenv("SHEETS_SPREADSHEET_ID")
    print(f"[lead] business_id={payload.business_id} sheet_override={sheet_override or 'none'} tenant_sheet_id={tenant_sheet_id or 'none'}")
    conversation_id = payload.conversation_id or str(uuid.uuid4())
    storage.ensure_conversation(conversation_id)
