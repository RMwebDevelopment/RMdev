...
class FakeAIClient(BaseAIClient):
    ...
    GENERIC_REPLIES = [
        "Thanks for reaching out. I can help find properties or get you in touch with our specialists.",
        "Happy to assist. I'll gather a few details so our team can follow up with you directly.",
        "Great question. I can pull up some listings for you, or we can discuss your specific search criteria.",
    ]
    ...
    def generate(self, messages: List[Dict[str, str]]) -> str:
        ...
        lead_capture = "no"
        next_step = "ask_need"
        summary = "Visitor has a general property question."
        urgency = derive_urgency(messages[-1]["content"])

        if any(keyword in user_message for keyword in ("book", "tour", "visit")):
            intent = "book"
            lead_capture = "yes"
            next_step = "ask_contact"
            summary = "Interested in a tour."
            reply = "I can help coordinate a showing. I'll just need a few quick details so our team can schedule that for you."
        elif any(keyword in user_message for keyword in ("price", "cost", "budget")):
            intent = "pricing"
            lead_capture = "yes"
            next_step = "ask_contact"
            summary = "Asking about pricing or budget."
            reply = "I can definitely help find homes in your range. What contact info should I pass to our agents to get you a full list?"
        else:
            reply = random.choice(self.GENERIC_REPLIES)
            if assistant_turns >= 2:
                lead_capture = "yes"
                next_step = "ask_contact"
                summary = "Continuing conversation, prompting for follow-up details."
...
def enforce_guardrails(
    ...
    """Ensure lead capture prompts fire on critical property phrases."""
    text = user_message.lower()
    updated = routing.copy()

    updated["urgency"] = derive_urgency(user_message)

    if any(term in text for term in LEAD_TERMS):
        ...
        updated.setdefault("summary", "High intent property inquiry")
        updated["next_step"] = "ask_contact"
    elif assistant_turns >= 3 and updated.get("lead_capture", "no") != "yes":
        updated["lead_capture"] = "yes"
        updated.setdefault("summary", "Providing follow-up contact prompt")
        updated["next_step"] = "ask_contact"
