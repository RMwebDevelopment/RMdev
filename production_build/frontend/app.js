(() => {
    ...
    function appendMessage(role, text, options = {}) {
        ...
        if (role === 'assistant') {
            // Extract <imageN src="..."> tokens into a simple carousel.
            const images = [];
            const imageRe = /<image\\d+\\s+src=\"([^\"]+)\"\\s+alt=\"([^\"]*)\"\\s*><\\/image\\d+>/gi;
            let cleaned = text;
            cleaned = cleaned.replace(imageRe, (_, src, alt) => {
                images.push({ src, alt });
                return '';
            });

            const textSpan = document.createElement('span');
            textSpan.textContent = cleaned.trim();
